...
<div class="card">
	...
	<div class="card-body">
		<form action="{{ route("acl.update",$role->id)}}" method="post" id="add_role" class="validate">
      <input name="_method" type="hidden" value="PUT">
      {{-- <input type="text" name="id" value="{{ $role->id }}" /> --}}
		{{ csrf_field() }}
		<div class="form-group">
			<label>@lang('acl::add_role.role_name'):</label>
			<input name="role_name" value="{{ $role->name }}" id="role_name" type="text" class="form-control" placeholder="@lang('acl::add_role.role_name')" required="">
		</div>
		<div class="row">
			@php($nr=0)
				...
				<div class="form-group">
					...
					<select name="permissions[]" class="form-control multiselect" multiple="multiple" data-fouc>
						@foreach ($package['permissions'] as $key => $permision)
						        <option @if (in_array($permision, $role_permissions)) selected="selected" @endif value="{{ $permision }}">@lang($permision)</option>
						@endforeach
					</select>
				</div>
			</div>
